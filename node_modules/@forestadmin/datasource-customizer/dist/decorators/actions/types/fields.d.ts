...
export type Handler<Context = unknown, Result = unknown> = ((context: Context) => Promise<Result>) | ((context: Context) => Result);
...
export type SearchOptionsHandler<Context = unknown, TValue = string> = ((context: Context, searchValue: string) => DropdownOption<TValue>[]) | ((context: Context, searchValue: string) => Promise<DropdownOption<TValue>[]>);
type LimitedValueDynamicFieldConfiguration<Context, TWidget, TValue = string> = {
    widget: TWidget;
    options: DropdownOption<TValue>[] | Promise<DropdownOption<TValue>[]> | SearchOptionsHandler<Context, TValue>;
};
